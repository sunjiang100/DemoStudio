...
__task		void 	task_send_CAN (void) 
{
	...
	{
		TC.CANSend = 0;
		os_dly_wait(5);		// 修改延时值时，请修改CanOverTimeCnt的判断值
		CanBus.eTaskSend();
...
void			CanBusCls::eTaskSend(void)
{
	...
	CanOverTimeCnt++;			// +50ms
	SendTickCnt++;				// +50ms
 	if( (jt_auth_ok_flag!=0)&&(TestCanBus==TRUE)&&(gCnt.CanBusNumber>0) )
	{	
...
void			CanBusCls::eTaskReceive(void)
{
	...
	if(SaveFlag==TRUE)
	{
		for(i=0;i<PageCnt;i++)
		{
			...
			DFF.PageProgramWaitOK(Addr,&TemporaryCanData[i*252],5);
		...
		{			
			if(FlashPageCnt.CanBusDataRead<FlashPageCnt.CanBusDataSave)
			{
				...
				if( (LastPageCanData>0)&&(FlashPageCnt.CanBusDataRead==FlashPageCnt.CanBusDataSave) )
				{
					...
					Len = strlen((char*)Tp.Buffer);
					Int2Str( gCnt.CanBusNumber, (char*)&Tp.Buffer[Len]);					
